    ...
    public static function WalletEdit($params = [])
    {
        ...
        foreach($money_field as $v)
        {
            ...
            if($wallet[$v['field']] != $data[$v['field']])
            {
                ...
                $log_data['msg'] = $admin['username'].'管理员操作 [ '.self::$money_type_list[$v['money_type']]['name'].'金额'.$msg.$log_data['operation_money'].'元 ]'.$operation_msg;
                ...
                {
                    MessageService::MessageAdd($wallet['user_id'], '账户余额变动', $log_data['msg'], BaseService::$business_type_name, $wallet['id']);
                }
            }
        }
