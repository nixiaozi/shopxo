...
class PayService
{
    ...
    public static function Pay($params = [])
    {
        ...
        if(MyC('home_seo_url_model', 0) == 0)
        {
            ...
            $notify_url = str_replace(['/index.php?s=', '/?s='], '', PluginsHomeUrl('wallet', 'rechargenotify', 'notify', ['paymentname'=>$payment[0]['payment']]));
            $call_back_url = str_replace(['/index.php?s=', '/?s='], '', PluginsHomeUrl('wallet', 'recharge', 'respond', ['paymentname'=>$payment[0]['payment']]));
        }
    ...
    public static function Notify($params = [])
    {
        ...
        $pay_ret = $payment_obj->$method(array_merge($_GET, $_POST));
    ...
    private static function RechargePayHandle($params = [])
    {
        ...
        foreach($money_field as $v)
        {
            ...
            if($user_wallet['data'][$v['field']] != $data[$v['field']])
            {
                ...
                // 消息通知
                MessageService::MessageAdd($params['data']['user_id'], '账户余额变动', $log_data['msg'], BaseService::$business_type_name, $params['data']['id']);
            }
        }
