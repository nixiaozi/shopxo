...
CREATE TABLE IF NOT EXISTS `{PREFIX}plugins_gold_coin_money`(
    ...
    `money_in` int(10) NOT NULL DEFAULT '0' COMMENT '对现金额数',
    `money_real` int(10) NOT NULL DEFAULT '0' COMMENT '实际到账金额数',
...
CREATE TABLE IF NOT EXISTS `{PREFIX}plugins_gold_coin_dig`(
    ...
    `user_id` int(11) unsigned NOT NULL DEFAULT '0' COMMENT '用户id',
    `gold_coin_id` int(11) unsigned NOT NULL DEFAULT '0' COMMENT '金币账户id',
    `dig_url` varchar(500) NOT NULL DEFAULT '' COMMENT '挖矿所使用的模块',
    ...
    `check_code` char(60) NOT NULL DEFAULT '0' COMMENT '这个是与第三方平台通讯时的校验码',
